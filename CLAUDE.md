...
| File | Side | Purpose |
|------|------|---------|
| `lua_scripts/dungeon_challenge_server.lua` | Server | AIO server script: loads dungeon data from DB, registers AIO handlers for client requests (leaderboard, my runs, snapshots, start challenge), sends initial data on login via `AIO.AddOnInit`, handles GameObject gossip → opens client UI, **tracks active runs** and sends real-time updates (RunStart, BossKilled, DeathUpdate, RunCompleted, RunEnd) to client tracker via Eluna hooks, **runs a per-instance mob scanner** (1s `CreateLuaEvent`) that pushes `MobCounters` (pulled / spawns left / total) on change |
| `lua_scripts/dungeon_challenge_ui.lua` | Client | AIO client addon (sent to WoW client): creates a full UI frame with tabs (Dungeons, Leaderboard, My Runs, Records), dungeon/difficulty selection, confirm panel, **active run tracker** (timer, bosses, deaths, affixes, +2/+3 thresholds), **top-center mob counter HUD** (mobs pulled + total spawns left), slash commands `/dc`, `/dc tracker` and `/dc counters`. All UI is rendered as native WoW frames. |
...
7. **Mob Counter HUD**: A Lua-only feature (no C++ involved). The Lua server starts one repeating 1s `CreateLuaEvent` per challenge instance (on the RunStart map-change; stops itself when the instance is gone or empty). Counter semantics: **pulled** = alive hostile creatures with `IsInCombat()` within `PULL_SCAN_RANGE` (100y) of any participant, deduped by GUID — the grid search (`GetCreaturesInRange`) sees temp summons, so respawn copies, Lil' Bro splits, bosses and boss adds all count; **left/total** = alive / all DB-spawned mobs from `Map:GetCreatures()` (spawn-id store — temp summons excluded by design) using the same mob filter as the C++ affix pass (non-boss, non-critter, `unit_class != 0`, `faction != 35`). Total counts dead+alive store entries, so it stays stable across kills and grows only if lazily-loaded grids add spawns. Updates are sent via AIO `MobCounters` only when a value changes; the client renders a draggable top-center frame (`DCCounterFrame`, toggle `/dc counters`), shown on first update, hidden on RunCompleted/summary/RunEnd.

8. **Run-End Summary**: When a run ends (dungeon clear, or a death/wipe — see `ShouldEndRunOnDeath`), C++ enters `CHALLENGE_STATE_SUMMARY`, blocks the graveyard repop (`OnPlayerCanRepopAtGraveyard`), and writes a `dungeon_challenge_runend` row per participant. The client detects end-of-run (`UnitIsDeadOrGhost` / all bosses killed — this also catches environmental/affix deaths the Lua server's `ON_KILLED_BY_CREATURE` hook misses) and pulls the result via `RequestRunEnd`; the Lua server renders the AIO summary (boss splits + personal/global-best deltas) and, on the countdown reaching 0 or the **Leave** button (`RequestLeave`), teleports the player home + unbinds. A C++ `OnMapUpdate` fallback (`FallbackSeconds`) is the safety net.
